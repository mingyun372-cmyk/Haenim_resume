...
function App(){
 ...
 return <main>
  ...
  {projects.map((p,i)=><section id={`project-${i+1}`} className="section project-page" key={p.title}><div className="project-page-number">PROJECT 0{i+1}</div><div className="project-page-content"><div className="meta"><span>{p.year}</span><span>{p.company}</span><span>{p.genre}</span></div><h2>{p.title}</h2><p>{p.summary}</p><div className="tags">{p.tags.map(t=><span key={t}>{t}</span>)}</div></div><div className={`project-page-proof${p.outputs?' output-cards':''}`}><span>KEY OUTPUT</span>{p.outputs?<div className="output-list">{p.outputs.map(output=><div className="output-card" key={output.label}><strong>{output.label}</strong><p>{output.text}{output.highlight&&<em>{output.highlight}</em>}</p></div>)}</div>:<b>{p.proof}</b>}</div><div className={`project-media${p.images?' has-images':''}`} aria-label={`${p.title} 관련 이미지 영역`}>{(p.images??[null,null]).map((image,index)=><div key={image?.src??index}>{image?<img src={image.src} alt={image.alt}/>:<span>IMAGE 0{index+1}</span>}</div>)}</div></section>)}
