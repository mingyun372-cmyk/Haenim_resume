...
function App(){
 ...
 return <main>
  <header className="nav"><a className="brand" href="#top">Hae-nim<span>.</span>Kang</a><nav className={menu?'open':''}>{['projects','skills','career','contact'].map((id,i)=><button key={id} onClick={()=>go('#'+id)}>{['프로젝트','역량','경력','연락'][i]}</button>)}</nav><button className="menu" aria-label="메뉴" onClick={()=>setMenu(!menu)}>{menu?<X/>:<Menu/>}</button></header>
  <section id="top" className="hero"><div className="eyebrow"><span/> SENIOR GAME SCENARIO WRITER</div><h1>이야기를 쓰고,<br/><em>플레이로 완성합니다.</em></h1><p className="lead">강해님 · 시나리오 / 퀘스트 / 콘셉트 기획</p><p className="intro">RPG부터 SNG까지 다양한 장르에 걸쳐<br/>세계관과 캐릭터를 설계하고 수백 개의 퀘스트를 실제 게임 데이터로 완성해 온<br/>내러티브 주력 기획자입니다.<br/>2008-2009 네이버 게임 파워블로거 경력이 있으며<br/>현재는 스토리 위주의 고전게임 플레이 실황을 방송하고 있습니다.</p><div className="hero-actions"><button className="primary" onClick={()=>go('#projects')}>대표 프로젝트 보기 <ChevronRight/></button></div></section>
  <section id="projects" className="section"><div className="section-head"><div><span className="kicker">SELECTED WORK</span><h2>대표 프로젝트</h2></div><p>역할, 산출물, 규모가 한눈에 보이도록 핵심 작업만 선별했습니다.</p></div><div className="stats project-stats"><div><b>10년 9개월</b><span>게임 기획 경력</span></div><div><b>370,000자</b><span>단일 프로젝트 메인 퀘스트</span></div><div><b>파트 리드</b><span>설정·시나리오 검수 경험</span></div></div><div className="filters">{filters.map(f=><button className={filter===f?'active':''} onClick={()=>setFilter(f)} key={f}>{f}</button>)}</div><div className="project-list">{visible.map((p,i)=><article className="project" key={p.title}><div className="project-index">0{i+1}</div><div className="project-main"><div className="meta"><span>{p.year}</span><span>{p.company}</span><span>{p.genre}</span></div><h3>{p.title}</h3><p>{p.summary}</p><div className="tags">{p.tags.map(t=><span key={t}>{t}</span>)}</div></div><div className="proof"><span>KEY OUTPUT</span><b>{p.proof}</b></div></article>)}</div></section>
