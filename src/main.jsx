...
const projects = [
  ...
  {year:'2011–2012', title:'드라고나 온라인', company:'라이브플렉스', genre:'MMORPG', tags:['시나리오','퀘스트'], summary:'드라고나 온라인 라이브 서비스에 참여하여 신규 지역 대규모 업데이트를 위한 메인 퀘스트 300여 개를 단독으로 제작하고 데이터 관리 업무를 담당했습니다.', proof:'50-59레벨 업데이트 퀘스트 300여 개'}
]
const skills = [
...
function App(){
 ...
 return <main>
  ...
  <section id="career" className="section"><div className="section-head"><div><span className="kicker">EXPERIENCE</span><h2>경력 타임라인</h2></div><p>종합출판사 편집자로부터 게임 내러티브 기획자로 전직하여<br/>라이브 MMORPG, 캐주얼 모바일, 액션 RPG, SNG에 샌드박스까지 이르는 다양한 장르를 경험했습니다.</p></div><div className="timeline">{career.map(c=><div key={c[0]}><time>{c[0]}</time><h3>{c[1]}</h3><p>{c[2]}</p></div>)}</div><div className="note"><strong>기반 역량</strong><p>국어국문학 전공 · 출판편집 경력 · 전문 교정/교열 · 일본어 중급 · Unity 플레이 테스트/데이터 입력 경험</p></div></section>
