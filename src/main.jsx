...
function App(){
 ...
 return <main>
  <header className="nav"><a className="brand" href="#top">Hae-nim<span>.</span>Kang</a><nav className={menu?'open':''}>{['projects','skills','career','contact'].map((id,i)=><button key={id} onClick={()=>go('#'+id)}>{['참여 프로젝트','역량','경력','연락'][i]}</button>)}<a className="resume-link" href={`${import.meta.env.BASE_URL}resume.html`}>이력서</a></nav><button className="menu" aria-label="메뉴" onClick={()=>setMenu(!menu)}>{menu?<X/>:<Menu/>}</button></header>
